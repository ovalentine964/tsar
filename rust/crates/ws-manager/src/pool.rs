...
impl ConnectionPool {
    ...
    pub fn add(&mut self, connection: WsConnection) -> TsarResult<uuid::Uuid> {
        if self.connections.len() >= self.max_connections {
            return Err(tsar_core::error::TsarError::Internal(format!(
        ...
        tracing::info!(connection_id = %id, pool_size = self.connections.len(), "Connection added to pool");
        Ok(id)
    }
    ...
    pub async fn remove(&mut self, id: &uuid::Uuid) -> TsarResult<()> {
        if let Some(mut conn) = self.connections.remove(id) {
            ...
            tracing::info!(connection_id = %id, pool_size = self.connections.len(), "Connection removed from pool");
        }
        Ok(())
    }
