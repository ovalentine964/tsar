"""
TSAR Trend Detector — Multi-timeframe trend analysis with HH/HL/LH/LL detection.
...
class TrendDetector:
    """Multi-timeframe trend detector for TSAR strategy.
    ...
    def analyze(
        ...
        """Alias for detect() used by EntryPipeline."""
        return self.detect(d1_closes, h4_closes, h1_closes)
    ...
    def update_genome(self, new_genome: dict[str, Any]) -> None:
        ...
        if "ma_slow_period" in mutable:
            self._slow_period = mutable["ma_slow_period"].get("current", self._slow_period)
        if "trend_ma_fast" in new_genome:
            self._fast_period = new_genome["trend_ma_fast"]
        if "trend_ma_slow" in new_genome:
            self._slow_period = new_genome["trend_ma_slow"]
